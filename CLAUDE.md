...
- **Decks**: Custom slide system with pptxgenjs export + Puppeteer/Chromium PDF; runs on **gpt-5-mini** end-to-end (generate/edit/critique). **Generation uses a deterministic layout system (default-on):** the model emits a compact `deckdsl` XML block (one of ~14 tested, overflow-safe, brand-token-themed layouts — title/section/bullets/stats/twoColumn/quote/statement/featureGrid/agenda/timeline/bigStat/team/chart/closing, incl. a real SVG bar/line chart and initial-avatar team), which `src/lib/deck/dslToHtml.ts` renders to themed 960×540 HTML. This replaced free-form per-slide HTML (which produced blank/clipped/off-brand slides). Gated by `NEXT_PUBLIC_DECK_DSL` (kill-switch: `=false`). A fixed eval harness (`src/lib/deck/deckEval.ts`, `npm run deck:eval [--render]`) asserts every layout is non-empty/branded/footered/clip-free/contrast-safe at min & max content, so quality is measured, not eyeballed. An agentic **deck-refine** pipeline renders each slide to an image and runs a **lens-based vision critique → repair** loop (`src/lib/ai/deck/lenses.ts` composes modular per-dimension lenses — readability, overflow, hierarchy, accessibility, neatness, creativity, plus intent-grounded **brand-adherence** and **prompt-adherence** that activate when a brand/brief is supplied). Readability is also enforced deterministically at render: `enforceSlideContrast` (string pass, gradient-aware + strips leaked continuation backslashes) plus `contrastFix.ts`' runtime computed-contrast pass in the shadow-DOM renderers. (`src/lib/ai/deck/`, `src/lib/deck/`, `src/components/deck/`, `/api/deck-refine`)
