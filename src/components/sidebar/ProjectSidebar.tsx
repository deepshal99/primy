...
export function ProjectSidebar() {
  ...
  const { data: serverProjects, isLoading: isLoadingProjects } = useProjects();
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-2 pb-2">
          {isLoadingProjects && projects.length === 0 ? (
            /* Loading skeletons — alternating-width ghost rows */
            <div className="flex flex-col gap-1 px-1 pt-1" aria-label="Loading projects">
              {[88, 65, 78, 55, 82, 70].map((w, i) => (
                <div
                  key={i}
            ...
            <div className="flex flex-col items-center justify-center py-14 px-4 animate-fade-in">
              {/* Small CSS folder illustration */}
              <div className="relative w-12 h-10 mb-4" aria-hidden>
                <div
                  className="absolute inset-x-0 bottom-0 h-[30px] rounded-[6px] bg-[#fff1eb] border border-[rgba(255,74,0,0.12)]"
                  style={{ boxShadow: "inset 0 1px 0 rgba(255,255,255,0.7)" }}
                />
                <div
                  className="absolute top-0 left-0 w-[22px] h-[8px] rounded-t-[4px] bg-[#fff1eb] border-t border-x border-[rgba(255,74,0,0.12)]"
                />
                <div className="absolute inset-x-3 bottom-[10px] h-[2px] rounded-full bg-[rgba(255,74,0,0.18)]" />
                <div className="absolute inset-x-4 bottom-[5px] h-[2px] w-[60%] rounded-full bg-[rgba(255,74,0,0.10)]" />
              </div>

              <p className="text-[13px] text-center mb-1 font-medium text-[#1a1a2e] font-heading">
                {search ? "No matches" : "No projects yet"}
              </p>
              <p className="text-[11.5px] text-center mb-4 text-[#95928E] leading-relaxed max-w-[200px]">
                {search
              ...
              {!search && (
                <button
                  onClick={handleNewProject}
                  className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-[12.5px] font-medium text-white bg-[#ff4a00] hover:bg-[#e54400] active:scale-[0.98] transition-all duration-150 shadow-[0_2px_8px_rgba(255,74,0,0.25)]"
                >
